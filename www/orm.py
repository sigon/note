...
def select(sql, args, size=None):
    ...
    with (yield from __pool) as conn:
        ...
        logging.info('rows return: %s' % (len(rs._result)))
        return rs
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        escaped_fields = list(map(lambda f: '`%s`' % f, columns))
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    @asyncio.coroutine
    def findAll(cls, where=None, args=None, page_index=1, page_size=10, **kw):
        ...
        if args is None:
            args = []
        count = yield from cls.findNumber('count(id)', where, args)
        page = Page(count, page_index, page_size)
        if count == 0:
            return {'page': page, '%ss' % cls.__table__: ()}
        orderBy = kw.get('orderBy', None)
        if orderBy:
            sql.append('order by')
            sql.append(orderBy)
        sql.append('limit')
        sql.append(str(page.page_size))
        sql.append('offset')
        sql.append(str(page.offset))
...
if __name__ == '__main__':
    ...
    def test():
        yield from create_pool(loop=loop,host='localhost', port=3306, user='root', password='', db='test')
